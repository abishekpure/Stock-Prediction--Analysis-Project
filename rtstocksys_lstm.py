...
def prep_data(prices: np.ndarray, window: int):
    """Prepare X, y and scaler for LSTM."""
...
def train_lstm(symbol: str, prices: np.ndarray, window: int, epochs: int = 8, batch_size: int = 32):
...
def lstm_forecast(symbol: str, prices: np.ndarray, window: int, steps: int):
    """Generate forecast using cached LSTM model."""
    if len(prices) == 0:
    ...
    # Optionally store metrics for display
...
for symbol in symbols:
    ...
    if len(prices) <= window:
        st.warning(f"Not enough data for {symbol} to train LSTM. Showing last prices only.")
    ...
    fig.add_trace(go.Scatter(x=future_time, y=forecast, mode="lines",
                             name="LSTM Forecast", line=dict(width=2, dash="dash", color="red")))
